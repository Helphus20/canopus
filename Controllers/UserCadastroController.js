...
*/
  ...
  try {
    ...
    const hash = await crypto.hash(senha, saltRounds);
    console.log('Senha hasherizada', hash);

    // Se tudo estiver ok, cadastra o usuário no banco
    UserModel.cadastrarUsuario(nome, hash, (err, result) => {
      if (err) {
